...
def find_path(start_point, goal_point, gs, obstacle_positions, obstacle_radius):
    ...
    start_node = Node1(start_point[0], start_point[1], 0, -1)
    ...
    while [start_node.x, start_node.y] != [goal_point[0], goal_point[1]]:
        ...
        if [current_node.x, current_node.y] == [goal_point[0], goal_point[1]]:
            ...
            wp_list_o.append([wp_node.x, wp_node.y])
        ...
        for move in valid_moves:
            ...
            new_cost = current_node.cost + math.dist(move, [current_node.x, current_node.y])+ math.dist(move, goal_point)
            ...
            new_node = Node1(move[0], move[1], new_cost, current_index)
            unvisited[new_index] = new_node 

    # path_x = [point[0] for point in wp_list_o]
    # path_y = [point[1] for point in wp_list_o]
        ...
        while rclpy.ok():
            ...
            for i in range(len(wp_list)):
                ...
                while current_distance_error >= distance_error_tolerance_m:
                    ...
                    dy = current_wp[1] - turtlebot_node.current_position[1]
                    current_distance_error = np.sqrt(dx**2 + dy**2)
                    current_trajectory.append((turtlebot_node.current_position[0], turtlebot_node.current_position[1]))
...
plt.title("Ideal Path vs Actual Path")
plt.xlabel("X")
plt.ylabel("Y")
plt.legend(["Ideal Path", "Actual Path"])
plt.show(block=False)
plt.pause(1)
