...
class Agent:
    def __init__(self, i, xi, yi, speed, N_tasks, color, obs_positions, obs_radius):
        self.i = i # my index
        self.x = xi # position
        self.y = yi # position
    ...
    def find_best_task(self, tasks):
        ...
        for i, task in enumerate(tasks): # search over all tasks
            if task.active: # don't do tasks that are done
                ...
                astar = Astar(world.x, world.y, self.speed, self.obs_positions, self.obs_radius)
                path, cost = astar.find_path([self.x, self.y], [task.x, task.y])
    ...
    def move_towards_goal(self, world): # move towards goal
        task = world.tasks[self.goal_task[1]]
...
for a in range(0,N_agents):
    agents.append(Agent(a, random.random() * world.x, random.random() * world.y, 2.0, N_tasks, colors[a%5], obs_positions, obs_radius))
